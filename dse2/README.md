...
- **公告**：管理员登入后可以发布置顶公告（显示在墙的最上方，红色边条样式），任何人都能看到；也能删除
- **回复**：任何访客都能对某张便条留言回复（匿名或实名），点便条下方「💬 回复」展开
- **管理员删帖 / 删公告 / 删回复**：右上角「🔒 管理员」输入密码后，便条、公告、回复上都会出现删除按钮。密码校验在服务器端完成（`ADMIN_PASSCODE` 环境变量），不会暴露在浏览器代码里
- 单张便条限制：文字 600 字以内，图片压缩后约 900KB 以内；单条回复 200 字以内；单条公告 300 字以内
- 数据库里最多保留最新 300 张便条、50 条公告，超过会自动清掉最旧的（避免数据库无限变大）
